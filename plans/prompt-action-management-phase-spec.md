...
- [ ] 将当前分散在 [`phone_agent/config/prompts.py`](../phone_agent/config/prompts.py) 、 [`phone_agent/config/prompts_zh.py`](../phone_agent/config/prompts_zh.py) 、 [`phone_agent/config/prompts_en.py`](../phone_agent/config/prompts_en.py) 、 [`phone_agent/config/prompts_thirdparty.py`](../phone_agent/config/prompts_thirdparty.py) 的提示词文本，重构为可组合、可裁剪、可注入策略的提示词基础设施。
- [ ] 建立统一的动作注册与能力描述层，不再依赖多个文件中手写重复动作清单。
- [ ] 在 GUI 中新增“AI 可用动作管理”区域，允许用户通过勾选项控制哪些动作对 AI 可见、哪些动作允许运行时执行。
- [ ] 配置需持久化，并通过 [`gui/services/config_service.py`](../gui/services/config_service.py) 、 [`gui/services/task_service.py`](../gui/services/task_service.py) 、 [`main.py`](../main.py) 传递到 Agent 运行时。
- [ ] 在执行层增加硬约束白名单，保证即使模型输出了被禁用动作，也不会真正执行。
- [ ] 提示词层同步做软约束，使模型尽量只规划已启用动作。
- [ ] Android ADB、HarmonyOS HDC、iOS XCTest 三条链路都能对动作能力进行统一描述，但允许平台差异化可用性。
- [ ] 保持现有 CLI 与 GUI 的主流程兼容，避免一次性打断当前可用能力。
...
- [ ] 新增动作注册层与元数据结构。
- [ ] 将现有动作迁移到注册表中声明。
- [ ] 为每个动作补齐分类、描述、平台支持与默认策略。
- [ ] 提供面向 Prompt、GUI、Runtime 的三个导出接口。

### 验收标准

- [ ] 不看 handler 映射，也能从注册表完整列出动作能力。
- [ ] GUI 与 Prompt 都能从注册表读取动作清单。
- [ ] 平台过滤结果稳定可测。
...
- [ ] 新建 Prompt Builder 体系。
- [ ] 重构中文提示词生成逻辑。
- [ ] 重构英文提示词生成逻辑。
- [ ] 重构第三方提示词生成逻辑。
- [ ] 用统一接口替换当前直接引用大段字符串常量的方式。
- [ ] 保留旧导出函数作为兼容层，避免一次性破坏调用方。

### 验收标准

- [ ] 提示词文本可按语言、平台、第三方模式、动作开关动态生成。
- [ ] 动作协议段落不再出现多份手写拷贝。
- [ ] 默认生成结果与现有行为在主流程上保持兼容。
...
- [ ] 在配置服务中加入动作集合字段。
- [ ] 完成 `.env` 读写、JSON 容错、默认值回填。
- [ ] 在 [`main.py`](../main.py) 中加入动作策略 CLI 参数。
- [ ] 将动作策略注入 [`AgentConfig`](../phone_agent/agent.py) 与 [`IOSAgentConfig`](../phone_agent/agent_ios.py) 。
- [ ] 保证 CLI 单独运行与 GUI 子进程运行两种模式都生效。

### 验收标准

- [ ] CLI 可显式指定动作白名单。
- [ ] GUI 启动任务时能把动作策略传入子进程。
- [ ] 无配置时能稳定回退到默认动作集。
...
- [ ] 在 Android/HarmonyOS 执行器中加入动作允许性检查。
- [ ] 在 iOS 执行器中加入动作允许性检查。
- [ ] 对被禁动作返回明确错误消息，并反馈给对话上下文。
- [ ] 统一错误码或错误消息格式，便于后续日志分析。
...
- [ ] 兼容旧 `.env` 配置。
- [ ] 为旧配置生成默认动作策略。
- [ ] 更新 [`README.md`](../README.md) 、 [`README_en.md`](../README_en.md) 与相关文档。
- [ ] 更新 [`docs/ai_tools_reference.md`](../docs/ai_tools_reference.md) ，说明动作策略与平台能力。
- [ ] 说明 GUI 动作勾选的作用边界：AI 可见不等于一定执行，最终仍受运行时白名单控制。
...
- [ ] 新增动作只需在注册表中声明，即可自动进入 Prompt、GUI 与 Runtime 策略链路。
- [ ] 用户可在 GUI 中稳定配置动作启用状态。
- [ ] 配置可持久化、可迁移、可回退。
- [ ] 提示词与执行器不再维护两份独立动作真源。
- [ ] 被禁动作不会真实执行。
- [ ] 三个平台的动作能力边界清晰可见。
- [ ] 测试、文档、CLI、GUI 四条链路同步收口。
...
- [ ] GUI 进入设置页时，从持久化配置加载动作策略，并与注册表做一次一致性校验。
- [ ] 启动任务时，配置服务输出“当前平台有效策略”，通过 CLI 参数注入任务子进程。
- [ ] Agent 初始化时生成两份有效集合：AI 可见动作集 与 Runtime 允许动作集。
- [ ] Prompt Builder 仅向模型暴露 AI 可见动作集，减少无效规划。
- [ ] 执行器最终只接受 Runtime 允许动作集，形成强约束闭环。
- [ ] 执行结果与拒绝原因都进入上下文，模型可持续自纠错重规划。

## 2. 策略优先级与决策顺序

- [ ] 优先级 P1 平台能力上限：动作必须先被平台注册表标记为支持。
- [ ] 优先级 P2 Runtime 白名单：动作必须在运行时允许集合内。
- [ ] 优先级 P3 AI 可见集合：动作可被模型规划但不等于必然执行。
- [ ] 优先级 P4 提示词模板策略：仅影响模型偏好，不改变运行时硬边界。
- [ ] 优先级 P5 模型自由决策：只能在前述边界内选择动作。

- [ ] 目标态计算规则一：有效 Runtime 动作集 = 平台支持动作集 与 Runtime 配置动作集 的交集。
- [ ] 目标态计算规则二：有效 AI 可见动作集 = 有效 Runtime 动作集 与 AI 可见配置动作集 的交集。
- [ ] 目标态计算规则三：当 AI 可见配置缺失时，默认回落到有效 Runtime 动作集。
- [ ] 目标态计算规则四：任何配置都不得突破平台注册表声明的能力上限。
...
- [ ] 兼容 C1 保持现有 CLI 参数有效，新增参数采用可选方式渐进接入。
- [ ] 兼容 C2 保持 GUI 旧配置可读，迁移后写回新配置结构。
...
- [ ] 场景 S1 用户在 GUI 禁用 `Call_API` 后，模型输出该动作会被稳定拒绝，且任务继续可恢复。
- [ ] 场景 S2 用户仅开放导航与输入动作时，Prompt 内容只展示对应动作。
- [ ] 场景 S3 Android 可用动作在切换到 iOS 后自动过滤，不产生无效勾选残留执行。
- [ ] 场景 S4 升级前旧配置可直接启动任务，系统自动生成默认动作策略。
