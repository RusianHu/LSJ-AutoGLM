...
在 Android ADB 模式下，当你不知道目标应用包名时，先用这个动作查询。执行结果会返回候选包名与 Activity，下一步应直接使用 Launch 并传入包名。

### 2. 启动应用
 do(action="Launch", app="应用包名")
在 Android ADB 模式下，请优先直接提供应用包名，例如 com.android.settings。系统会先将输入视为包名启动，再回退到内置映射。
...
3) 若当前不在目标 App：优先 do(action="Launch", app="目标包名")
