...
### 1. 查找应用包名
 do(action="Find_App", query="关键词")
在 Android ADB 模式下，当你不知道目标应用包名时，必须先用这个动作查询。执行结果会返回候选包名与 Activity，下一步应直接使用 Launch 并传入包名。不要猜测应用名后直接 Launch。

### 2. 启动应用
 do(action="Launch", app="应用包名")
在 Android ADB 模式下，请优先直接提供应用包名，例如 com.android.settings。如果还不知道准确包名，绝不要用模糊应用名调用 Launch，而应先调用 Find_App。系统会先将输入视为包名启动，再回退到内置映射。
...
用户任务: 找到设置的包名
正确输出: do(action="Find_App", query="settings")

用户任务: 打开设置，但还不知道包名
正确输出: do(action="Find_App", query="settings")

用户任务: 已经知道设置包名，打开设置
正确输出: do(action="Launch", app="com.android.settings")
...
用户任务: 打开设置查看存储
<think>当前还不知道设置的准确包名，先查询包名更稳妥</think>
<answer>do(action="Find_App", query="settings")</answer>
...
- do(action="Back") 返回
- do(action="Home") 主屏幕
- do(action="Find_App", query="关键词") 查包名
...
3) 若当前不在目标 App：如果还不知道目标包名，先 do(action="Find_App", query="应用关键词")；只有已经知道包名时才 do(action="Launch", app="目标包名")
